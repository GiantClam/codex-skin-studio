...
运动动作不能只依赖单张姿势图的平移、缩放或镜像。`running-right`、
`running-left` 和目标版本中的 `running` 至少要先生成四个具有四肢变化的
关键帧：前脚着地、抬膝经过、换脚着地、腾空换步；手臂必须与腿形成反向
摆动。工作动作也应生成手部和视线变化，而不是把同一张工作图复制到整行。

### 动作连续性验收标准

Skill 生成动作时必须先生成语义关键帧，再组装图集：

- 可使用等宽横向多格动作条，也可逐帧调用生图；每格必须是完整人物，不能带边框、标签或文字；
- 多格动作条必须先裁切成独立帧，再进入图集；所有帧保持相同镜头比例、透明安全边距和主体身份；
- 允许在已经具备完整语义循环后重复关键帧闭合循环，不允许用一张图的平移、缩放、镜像直接填满动作行；
- 视觉复核必须能用自然语言解释相邻帧的状态变化；解释不清时，整行动作重新生成；
- `waving` 必须包含手放下、抬手、举手、回落；`jumping` 必须包含蓄力、起跳、最高点、下降、落地；
- `failed` 必须包含惊讶、失落、瘫坐、恢复；`waiting` 必须改变眼睛、手、耳朵和尾巴；
- 工作或审查状态必须改变手部、视线、头部角度和工作道具；16 个视线方向必须连续转向，不能用静态正面帧镜像填充。

像素校验只能证明透明度、尺寸、安全边距和像素差异；Vision 才能证明动作语义连续性。两者必须同时通过。
如果无损 WebP 回读后出现完全透明像素的 RGB 残留，必须保留已校验的透明 PNG fallback。
...
- 图集尺寸可被 8 和 9 整除；
- 每个动作行至少包含有效帧；
- 每个动作和 look-direction 行的每一对相邻帧必须存在可见像素差异，且至少有一对达到可读动作阈值，禁止把同一张静态图复制到整行动作帧；
- 角色没有被明显裁切；
- 四角透明；
- 帧间头身比例和脚底基线稳定。
...
2. **Vision 验证**：卡通化、拟人化、大头小身体、角色一致性、表情可读性、动作连贯性和绿边。机器验证通过不代表视觉验收通过。
...
- 自动化回归测试为 `89/89`，Windows 路径、PowerShell 设置入口和安装契约测试为 `3/3`；Pet 校验新增动作帧差异检查，静态重复帧会以 `PET_ANIMATION_INVALID` 拒绝；最新 Windows workflow `29590065014` 已从 Microsoft Store `OpenAI.Codex` 安装包读取 `hatch-pet/references/codex-pet-contract.md`，通过 `verify-pet-contract.mjs` 独立确认 v2 格式，再由 `verify-pet-desktop.mjs` 要求真实 selected-row 和 loaded-sprite postcondition；
