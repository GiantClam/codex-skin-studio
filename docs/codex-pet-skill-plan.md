...
> 状态：MVP 实施中；Codex V2 Desktop 契约已从随应用提供的 `hatch-pet` Skill 冻结，原生 Refresh、选择和 `/pet` 端到端验收仍待完成
> 研究日期：2026-07-17
...
当前 `switch-paired.mjs` 的成功状态明确为 `theme-applied-pet-refresh-required` 或 `theme-scheduled-pet-refresh-required`。由于没有公开桌面端程序化选择接口，不能报告 `paired-active`；必须由真实 Refresh、选择和 `/pet` 结果确认。
...
- 本机 ChatGPT Desktop `26.715.21316` 已发现官方 `hatch-pet` 资源契约，但当前 Renderer DOM 和 `~/.codex/pets` 尚未完成真实 Refresh、选择、`/pet` 端到端验收；Windows 仍缺少本机手工验证。

契约冻结已经解除，剩余安全门是应用行为验收：工具可以生成和安装 v2 Pet，但在没有真实应用 postcondition 时仍只能报告 `refresh-required`，不能把本地文件安装误报为 Desktop Pet 已选中或正在运行。
