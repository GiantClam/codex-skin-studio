...
Skill 会分别生成 Hero 和 Pet 动作帧，分别校验，创建配套 Bundle，原子安装 Pet，并应用主题。切换命令随后通过版本化的可见 Settings > Pets 适配器执行 Refresh 和选择。原生成功状态为 `theme-applied-pet-selected`；UI 不可用时会真实降级为 `theme-applied-pet-refresh-required`。选择后应确认匹配的 Pet Overlay。当前实测 Desktop 版本不识别 `/pet`，不能把该命令的错误响应误报为唤醒成功。
