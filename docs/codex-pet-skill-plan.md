...
> 状态：MVP 实施中；Codex V2 Desktop 契约已冻结，macOS 可见 Settings > Pets Refresh 和选择适配器已完成真实验证，`/pet` 动画唤醒与 Windows 手工验收仍待完成
> 研究日期：2026-07-17
...
- 一次真实 ChatGPT Desktop Refresh、选择和 `/pet` 唤醒验证；当前 macOS 已完成 Refresh 和选择，`/pet` 仍是剩余验收项。

MVP 不包含：

- 通过配套 Bundle 一次性安装并尝试选择匹配的 Pet；
...
了 Web 上传规格以及 Desktop 的 `Settings > Pets > Refresh` 和 `/pet` 流程；
仍没有公开桌面端程序化选择 API。
...
5. 记录实际行顺序、尺寸和应用行为；macOS 已完成资源契约冻结，Windows 需执行相同版本的应用 Refresh 验证。
6. 生成 `contract.json`，并把未完成的原生 Refresh/选择结果保持为 `pending`，不能伪造为 `running`。
...
- 本机 ChatGPT Desktop `26.715.21316` 已发现官方 `hatch-pet` 资源契约；已通过当前 Renderer 的可见 Settings > Pets 控件真实完成 Refresh、匹配 Pet 选择和 selected-row postcondition，适配器版本为 `chatgpt-desktop-pets-settings-v1`；`/pet` 动画唤醒仍待记录，Windows 仍缺少本机手工验证。

契约冻结已经解除，剩余安全门是 `/pet` 动画行为和 Windows 手工验收。工具可以生成、安装并在当前 macOS UI 中确认 v2 Pet 已选择；没有 selected-row 或运行结果 postcondition 时仍必须报告 `refresh-required` 或 `pending`，不能把本地文件安装误报为 Desktop Pet 正在运行。
